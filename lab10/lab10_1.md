...
Keep the charts as simple and clean as possible. 
Different plots will only work with certain types of data. If you can not draw out the plot on paper, it will be difficult to code it. 
...
geom: plot type (bar,scatter,box,etc)
aesthetics: axis 
...
```
With glimpse, we can see the types of data we are working with).
...
x is used to show what you want on the x axis 
y is used what you want on the y axis 

specification of axis titles is part of the aesthetics 
...
use of the geom function is where you introduce the type of plot you are going to use. 
...
Note the 3 steps. Specify the data we want the plot to be made from. Specify the aesthetics (in this case the x and y axis). Specify the TYPE of plot style we want using the geom function 


Note that we are not putting pipes to connect each piece of the plot construction. We use a "+" instead 
...
```
In our data, we have both categorial and numeric data 
### 1. Scatter Plots
...
```r
ggplot(data=homerange, #specify the data being used to make the plot
...
Above, we are plotting mass and homerange. As biologist, we should expect the larger animals to travel farther distances, this prediction helps see what data we expect. The graph trend line even suggests that there is a true relationship between the variables used. 

Plots are built in layers. We use the + to indicate we are adding the next layer to the plot (aka going in b/w the steps in building our plot). 
...
In this code, if we have really big data and we want to make scatter plots, we can get the problem of overplotting (where the dots get squished together to which you cant distinguish the plots). genom_jitter adds random noise to seperate the data points out so the points are easier to read 
...
```r
ggplot(data=homerange, #specify the data being used to make the plot
       mapping=aes(x=log10.hra, y=log10.preymass))+#map the aesthetics
  geom_point(na.rm = T)+
...
When NA values are present in the data, geom removes the NA values so that they do NOT get plotted. By default the NAs are removed. Although we can specify the NAs to be removed. 
...
## 2 herbivore       227
```
Remember that count functions only work with the categorical variables. 
...
Using shortcuts here, counting the number of observations in a single variable. Because there is only variable we do not really need to use the "data="

These bar plots will NOT work with continious data. Must be using categorical data. 
...
geom_col allows us to compare 
specifying the y-axis first, flips the graph on its side. Can also use cord_flip to flip the data on its axis, depending on what you want
...
```r
mammals %>% 
  ggplot(aes(x=trophic.guild, y=taxon))+
  geom_bar(stat="identity")
```

![](lab10_1_files/figure-html/unnamed-chunk-23-1.png)<!-- -->
We have to use the log values because the raw data values would be too drastic to compare 
...
-10 is us specfying we want to the the top top 10 


If we have a x that is categorial and a y that is continous we will need to use geom col 
If we have x and y as both categorical (for count data) we can use geom bar 
