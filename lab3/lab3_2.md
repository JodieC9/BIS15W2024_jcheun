...
This code is saying: replace scientist in the hot springs data frame, with the same exact data but with scientist as a factor data type 
...
## [1] "Jill"  "Steve" "Susan"
```
Levels is a command to tell you how many different factors you have 

If we wanted to find the number of scientists... 
...
```r
names(fish) ##this code tells you the name of the variables in the fish file (aka the row names)
```
...
```r
nrow(fish) ##command shows you the number of rows in the data file
```
...
```r
head(fish) ## gives top obervations 
```
...
```r
tail(fish) ## gives bottom obeservations 
```
...
```r
table (fish$lakeid) ## this means make me a table from the fish data set of the lake id data
```
...
```r
little_fish <- filter(fish, length<=100) ##assigning a new object which represents the filtered data from the fish data set specifically of length that is less than 100 
little_fish
```
