...
```{r}
z=30 #do not use equal sign for object assignments 
q <- 42
```
...
```{r}
is.integer(my_numeric) #is my_numeric (the object) an integer?
```
...
3. How do you interpret this result? What does this mean about NAs?  
  ...
  RStudio is unable to calculate the mean of the data set if there is missing data. 
