...
We want to be able to run a shiny app directly from the script and away from the R markdown form. When you open the homerange app, it is not in Rmarkdown and you only need to press "run app". 

Shiny apps will NOT knit, so you wont be able to knit your hw. You need to make the app stand alone instead in a separate Rscript file. 
## Practice
...
```
But we want the reactivity part to allow us to change the taxon used to show the data 
```{r}
library(shiny)

ui <- fluidPage(titlePanel("Log 10 Homerange by Taxon"), #this adds a title to the app, and goes before the selectInput 
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    homerange %>% 
      filter(taxon==input$x) %>% #the reactive part of the shiny app is the filter function, in the app it will automatically filter to the taxon. You do not only have to assign reactivity to just x and y, you can assign it to other functions 
      ggplot(aes(x=log10.hra))+
      geom_density(fill="lightblue", oclor= "black", alpha=0.6)
  })
}
...
Chops up the shiny app into a dashboard setting and a tab bar on the side. 
...
```{r}
ui <- dashboardPage(
  dashboardHeader(title="Penguin"),
  dashboardSidebar(disable = T), #turns off the side bar 
  dashboardBody(
    ...
    sliderInput("pointsize", "Select the Point Size", min = 0.5, max = 3, value = 1, step = 0.5),
    plotOutput("plot", width = "500px", height = "400px"))#where we add the ui from the basic shiny app code 
)
...
shinyApp(ui, server)
```
Note that adding spaces between the code lines does not impact the code, and it makes it easier to see what is going on too. Note you need to copy past the basic code for the ui and server portions 
...
ui <- dashboardPage(
  ...
  dashboardBody(
  fluidRow(
  box(title= "Plot Option", width =3,#this shrinks down the box of the with variable select 
...
With the extra code the variable select and the graph itself are in different boxes 
...
```{r}
ui <- dashboardPage(
  dashboardHeader(title = "Sex Composition by Wolf Ppopulation"),
  dashboardSidebar(disable = T),
  dashboardBody(
    fluidRow(
    box(width=3, #controls box width
    selectInput("x", "Select Population", choices=unique(wolves$pop)),hr() #choices=unique, gives help when you have multiple observations to include without typing all of them out 
...
)#close the ui component of shiny

server <- function(input, output, session) {
  
  session$onSessionEnded(stopApp) #automatically stops the app when we close it 
...
shinyApp(ui, server)
```
Messing with the spaces, tabs, and orientation of each code line will not impact its function 
...
once you add the theme in the theme=shinytheme() to specify the theme of the plot you want and it will load up with that theme 
